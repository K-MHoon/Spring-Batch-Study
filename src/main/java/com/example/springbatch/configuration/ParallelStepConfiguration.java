package com.example.springbatch.configuration;

import com.example.springbatch.listener.StopWatchJobListener;
import com.example.springbatch.tasklet.ParallelStepCustomTasklet;
import lombok.RequiredArgsConstructor;
import org.springframework.batch.core.Job;
import org.springframework.batch.core.Step;
import org.springframework.batch.core.configuration.annotation.JobBuilderFactory;
import org.springframework.batch.core.configuration.annotation.StepBuilderFactory;
import org.springframework.batch.core.job.builder.FlowBuilder;
import org.springframework.batch.core.job.flow.Flow;
import org.springframework.batch.core.launch.support.RunIdIncrementer;
import org.springframework.batch.core.step.tasklet.Tasklet;
import org.springframework.batch.core.step.tasklet.TaskletStep;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.core.task.TaskExecutor;
import org.springframework.scheduling.concurrent.ThreadPoolTaskExecutor;

@Configuration
@RequiredArgsConstructor
public class ParallelStepConfiguration {

    private final JobBuilderFactory jobBuilderFactory;
    private final StepBuilderFactory stepBuilderFactory;

    @Bean
    public Job parallelStepJob() {
        return jobBuilderFactory.get("parallelStepJob")
                .incrementer(new RunIdIncrementer())
                .start(parallelStepFlow1())
                .split(parallelStepTaskExecutor()).add(parallelStepFlow2())
                .end()
                .listener(new StopWatchJobListener())
                .build();
    }

    @Bean
    public Flow parallelStepFlow2() {

        TaskletStep step2 = stepBuilderFactory.get("step2")
                .tasklet(parallelStepCustomTasklet())
                .build();

        TaskletStep step3 = stepBuilderFactory.get("step3")
                .tasklet(parallelStepCustomTasklet())
                .build();

        return new FlowBuilder<Flow>("flow2")
                .start(step2)
                .next(step3)
                .build();
    }

    @Bean
    public Flow parallelStepFlow1() {

        TaskletStep step1 = stepBuilderFactory.get("step1")
                .tasklet(parallelStepCustomTasklet())
                .build();

        return new FlowBuilder<Flow>("flow1")
                .start(step1)
                .build();
    }

    @Bean
    public Tasklet parallelStepCustomTasklet() {
        return new ParallelStepCustomTasklet();
    }

    @Bean
    public TaskExecutor parallelStepTaskExecutor() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(4);
        executor.setMaxPoolSize(8);
        executor.setThreadNamePrefix("async-thread-");
        return executor;
    }
}
